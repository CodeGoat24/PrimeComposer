...
def register_attention_control(model, controller, center_row_rm, center_col_rm, target_height, target_width, width, height, segmentation_map=None, pseudo_cross=False): 
    def ca_forward(self, place_in_unet):
        def forward(x, context=None, mask=None, encode=False, controller_for_inject=None, inject=False, layernum=None, main_height=None, main_width=None, ref_id_list=None):
            ...
            if inject and is_cross == False:
                ...
                orig_loc = torch.tensor((orig_loc), device=x.device).long()
                ...
                del orig_mask, orig_loc_masked, sim_fg
    ...
    def prompt_ca_forward(self):
        def forward(x, context=None, mask=None, encode=False, controller_for_inject=None, inject=False, layernum=None, main_height=None, main_width=None, ref_id_list=None):
            ...
            if exists(ref_id_list):
                ...
                B = sim.shape[0]
                H = int(math.sqrt(sim.shape[1]))
                W = H
                orig_mask = torch.zeros(H, W)
                ...
                orig_mask = orig_mask.to(torch.float16)
                mask = mask.to(sim.device)
                orig_mask = orig_mask.to(sim.device)
                mask[h:, :, :, ref_id_list] = orig_mask.unsqueeze(0).unsqueeze(-1)
    ...
    def register_recr(net_, count, place_in_unet):
        if 'CrossAttention' in net_.__class__.__name__:
            if net_.to_k.in_features != 1024:
                ...
                net_.forward = prompt_ca_forward(net_)
                return count
